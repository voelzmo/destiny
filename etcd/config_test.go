...
var _ = Describe("Config", func() {
	Describe("NewConfigWithDefaults", func() {
		It("applies the default values for fields that are missing", func() {
			...
			Expect(config.Secrets.Consul.AgentKey).To(Equal(consul.AgentKey))
			Expect(config.Secrets.Consul.AgentCert).To(Equal(consul.AgentCert))
			Expect(config.Secrets.Consul.ServerKey).To(Equal(consul.ServerKey))
			Expect(config.Secrets.Consul.ServerCert).To(Equal(consul.ServerCert))
